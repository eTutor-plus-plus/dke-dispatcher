...
public class XMLUtil {
	 ...
	 * @param whitespace Denotes if whitespaces are preserved so that a number of empty spaces are treated as belonging to a text node.
	 * @return The parsed XML document.
	 * @throws SAXException if a parsing exception occured, basically due to a String which is not wellformed with regard to XML.
	 * @throws IOException if an IOException occured when parsing.
	 */
	public static XMLDocument parse(String xml, boolean whitespace) throws SAXException, IOException {
	 ...
	 * @return The parsed XML document.
	 * @throws SAXException if a parsing exception occured, basically due to a String which is not wellformed with regard to XML.
	 * @throws IOException if an IOException occured when accessing the URL or when parsing the content.
	 */
	 ...
	 * @return true if there is no validation error, false if there is at least one validation error.
	 ...
	 */
	public static boolean isValid(XMLDocument doc, String schemaString) throws IOException, SAXException, XSDException {
	 ...
	 * @return true if there is no validation error, false if there is at least one validation error.
	 * @throws IOException if an IOException occurs when trying to find the schema or when parsing the content. 
	 * @throws SAXException if a parsing exception occured, basically due to a String which is not wellformed with regard to XML. 
	 * @throws XSDException if the content can not be parsed as XML Schema.
	 */
	public static boolean isValid(XMLDocument doc, URL schemaURL) throws IOException, SAXException, XSDException {
	    return validate(doc, schemaURL).getListTrees().size() == 0;
	}
	/**
	 * Checks if an XML document is valid with regard to a schema, given by an <code>XMLSchema</code> object.
	 * 
	 * @param doc The XML document to check.
	 ...
	 * @throws XSDException if the content can not be parsed as XML Schema.
	 */	
	public static boolean isValid(XMLDocument doc, XMLSchema schema) throws IOException, SAXException, XSDException {
	 ...
	 * @throws XSDException if the content can not be parsed as XML Schema.
	 */
	public static XMLError validate(XMLDocument doc, String schemaString) throws IOException, SAXException, XSDException {
	 ...
	 * @throws XSDException if the content can not be parsed as XML Schema.
	 */
	public static XMLError validate(XMLDocument doc, URL schemaURL) throws IOException, SAXException, XSDException {
	 ...
	 * @throws XSDException if the content can not be parsed as XML Schema.
	 */	
	public static XMLError validate(XMLDocument doc, XMLSchema schema) throws XSDException, SAXException, IOException {
	 ...
	 * @throws IOException if an exception occured when writing to the file.
	 */
	...
	/**
	 * This can be used to generate a temp file.
	 * @param tempParameter The object holding meta information about the file to be generated.
	 * @return The generated temp file.
	 * @throws IOException if an exception occured when generating the temp file.
	 */
	public static File generateTempFile(FileParameter tempParameter) throws IOException {

	    String folder = tempParameter.getTempFolder();
		String prefix = tempParameter.getTempPrefix();
		String suffix = tempParameter.getTempSuffix();
		boolean deleteOnExit = tempParameter.isDeleteOnExit();
		boolean defaultOnError = tempParameter.isDefaultOnError();
	    ...
	    if (folder != null) {
			/*
			URL baseDir = XMLUtil.class.getResource("/.");
			tempDir = new File(baseDir.getFile(), folder);
			if (!tempDir.exists()) {
				tempDir.mkdir();
			}
			*/
	    	try {
				URL url = DatalogCoreManager.getResource(folder);
		...
		try {
			File resultFile = File.createTempFile(prefix, suffix, tempDir);
			if (deleteOnExit) {
				resultFile.deleteOnExit();
			}
			return resultFile;
		} catch (IOException e) {
		    // Checks if tempDir, respectively the desired folder has not been set at all, so that the default was already selected implicitly
	 ...
	 * @return true if no validation error could be detected, false if at least one validation error could be detected.
	 * @throws IOException if an internal IO exception occured when processing the XML document so that it can be validated.
	 * @throws DocumentException if an internal parsing exception occured when processing the XML document so that it can be validated.
	 * @throws SAXException if the DTD is not applicable itself.
	 */
	public static boolean isValid(XMLDocument doc, String dtdElementName, String dtdFile) throws IOException, DocumentException, SAXException {
		...
		Document docCopy = saxReader.read(new StringReader(xml));
	 ...
	 * @throws IOException if an exception occured when printing the contents of the XML node to the String.
	 */
	 ...
	 * @throws IOException if an exception occured when printing the contents of the XML node to the writer.
	 */
